...
OUT = ROOT / "docs" / "porjai"


MARKDOWN = """# Porjai - HIS Integration API Specification
...
def parse_table(lines: list[str], start: int) -> tuple[list[list[str]], int]:
    ...
    while i < len(lines) and lines[i].startswith("|"):
        if i != start + 1:
            rows.append([cell.strip().replace("\\`", "`") for cell in lines[i].strip("|").split("|")])
        i += 1
    return rows, i
...
def main() -> None:
    ...
    md_path = OUT / "Porjai_Abstract_HIS_API_v4_abs.md"
    docx_path = OUT / "Porjai_Abstract_HIS_API_v4_abs.docx"
