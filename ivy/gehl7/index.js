/**
 * gehl7/index.js — GE Patient Monitor HL7 TCP listener
 *
 * Accepts MLLP-framed HL7 v2.x messages from a GE B105/B125/B450/CARESCAPE
 * monitor and routes each OBX segment through obxMapper → parameter_aliases
 * → ivy_observations.
 *
 * Environment variables:
 *   IVY_HL7_PORT          TCP listen port (default 6000)
 *   IVY_HL7_HOST          Bind address    (default 0.0.0.0)
 *   IVY_HL7_DEVICE_ID     Logical device ID stored in DB (default ge-monitor-or1)
 *   IVY_MONITOR_IP        Monitor IP for connectivity ping
 *   IVY_MONITOR_PING_MS   Ping interval ms (default 10000)
 *   IVY_HL7_LOG_RAW       Log raw HL7 text: "1" enable / "0" disable (default 1)
 *   IVY_HL7_LOG_MAX_CHARS Max chars of raw HL7 logged per message (default 20000)
 */
...
const MONITOR_IP        = String(process.env.IVY_MONITOR_IP || process.env.GE_B105_IP || process.env.HL7_MONITOR_IP || "").trim();
...
const LOG_RAW_HL7_MAX_CHARS = Math.max(500, Number(process.env.IVY_HL7_LOG_MAX_CHARS || 20000));
...
function buildAck(controlId, accepted) {
  ...
  const safeId   = controlId || "1";
  return (
    `MSH|^~\\&|IVY|AIDAS|GE|OR1|${ts}||ACK|${safeId}|P|2.3\r` +
    `MSA|${msaCode}|${safeId}\r`
  );
}
...
function runPingOnce() {
  return new Promise((resolve) => {
    ...
    execFile("ping", args, { windowsHide: true, timeout: 1500 }, (err, stdout, stderr) => {
      const joined = `${String(stdout || "")}\n${String(stderr || "")}`;
      const rttMs  = parsePingRtt(joined);

      if (err) {
        resolve({ ok: false, rttMs, error: err.message || "ping_failed" });
        return;
      }
...
function startPingLoop() {
  ...
  pingTimer = setInterval(() => void refreshPingState(), Math.max(2000, PING_INTERVAL_MS));
}
...
function startGEHL7Server() {
  ...
  server = net.createServer((socket) => {
    connectedCount += 1;
    console.log(
      `[HL7] monitor connected: remote=${socket.remoteAddress}:${socket.remotePort} ` +
    ...
    socket.on("close", () => {
      ...
      console.log(
        `[HL7] monitor disconnected: remote=${socket.remoteAddress}:${socket.remotePort} ` +
        `local=${socket.localAddress}:${socket.localPort}`,
      );
    });
...
// Auto-start when module is loaded (same behaviour as original gehl7.js)
startGEHL7Server();

process.on("exit", () => stopPingLoop());
