...
def overlap_check(payload: StartTime, _: dict = Depends(current_user), database: Connection = Depends(connection)):
    start = payload.start_time // 900000 * 900000
    return {"ok": True, "start_time": start, "overlap": start_overlap(database, start)}


@router.post("/start")
def start_case(payload: StartCase, actor: dict = Depends(current_user), database: Connection = Depends(connection)):
    hn = payload.hn.strip()
    if not hn:
    ...
    with database.transaction():
        ...
        date = datetime.fromtimestamp(start/1000, ZoneInfo("Asia/Bangkok")).strftime("%Y%m%d")
        ...
        case_id = database.execute(
            """INSERT INTO cases (case_code,hn,start_time,device_capture_start_time,status,created_at,updated_at)
               VALUES (%s,%s,%s,%s,'active',%s,%s) RETURNING id""",
            (code,hn,start,capture,current,current),
        ).fetchone()["id"]
    ...
    return {"ok":True,"case_id":case_id,"case_code":code,"start_time":start,
...
def discharge(payload: Discharge, actor: dict = Depends(current_user), database: Connection = Depends(connection)):
...
@router.post("/archive")
def archive(payload: CaseId, _: dict = Depends(current_user), database: Connection = Depends(connection)):
    with database.transaction():
...
def update_start(case_id: int, payload: StartTime, _: dict = Depends(current_user), database: Connection = Depends(connection)):
    start = payload.start_time // 900000 * 900000
    with database.transaction():
...
def update_end(case_id: int, payload: EndTime, actor: dict = Depends(current_user), database: Connection = Depends(connection)):
...
def suggested_end(case_id: int, database: Connection = Depends(connection)):
