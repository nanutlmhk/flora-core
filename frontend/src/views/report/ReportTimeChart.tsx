...
function PressureMarker({
  ...
  const path =
    direction === "down"
      ? `M ${x - half} ${y - half} L ${x} ${y + half} L ${x + half} ${y - half}`
      : `M ${x - half} ${y + half} L ${x} ${y - half} L ${x + half} ${y + half}`;
  return (
    <path
      d={path}
  ...
  return (
    <div className="report-chart report-surface border-x border-t report-border" style={{ width: totalWidth, minWidth: totalWidth }}>
      <div style={{ display: "grid", gridTemplateColumns: `${labelColWidth}px ${chartWidth}px` }}>
        <div className="relative border-r report-border px-2 py-2 pr-8 text-[10px] leading-4 report-cell-text">
          ...
          <div className="pointer-events-none absolute inset-y-0 right-1 top-0 w-7">
            {yTicks.map(tick => (
              <span
                key={`ytick-${tick.value}`}
                className="absolute right-0 text-[8px] leading-none report-cell-text"
                aria-hidden="true"
                style={{ top: `${Math.max(0, tick.y - 4)}px` }}
              >
                {tick.value}
              </span>
            ))}
        ...
        <svg width={chartWidth} height={height} className="block">
          ...
          {yTicks.map(({ value, y }) => (
            <line
              ...
              y2={y}
              stroke={GRID_STROKE}
              strokeWidth={value % 40 === 0 ? 0.7 : 0.5}
              strokeDasharray={value % 40 === 0 ? undefined : "2 2"}
            />
          ))}

          {visible.spo2 ? spo2Segments.map(segment =>
            segment.length > 1 ? (
              <path key={`spo2-${segment[0].ts}`} d={buildPath(segment)} fill="none" stroke={SPO2_COLOR} strokeWidth={1} />
            ) : null,
          ) : null}
          ...
          {visible.nibp ? nibpSys.map(point => <PressureMarker key={`nibp-s-${point.ts}`} x={point.x} y={point.y} color={NIBP_COLOR} direction="up" />) : null}
          {visible.nibp ? nibpMap.map(point => <Dot key={`nibp-m-${point.ts}`} x={point.x} y={point.y} color={NIBP_COLOR} />) : null}
          {visible.nibp ? nibpDia.map(point => <PressureMarker key={`nibp-d-${point.ts}`} x={point.x} y={point.y} color={NIBP_COLOR} direction="down" />) : null}
          ...
          {visible.art ? artSys.map(point => <PressureMarker key={`art-s-${point.ts}`} x={point.x} y={point.y} color={ART_COLOR} direction="up" />) : null}
          {visible.art ? artMap.map(point => <Dot key={`art-m-${point.ts}`} x={point.x} y={point.y} color={ART_COLOR} />) : null}
          {visible.art ? artDia.map(point => <PressureMarker key={`art-d-${point.ts}`} x={point.x} y={point.y} color={ART_COLOR} direction="down" />) : null}
